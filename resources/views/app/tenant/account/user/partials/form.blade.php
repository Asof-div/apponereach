...
<form id="user_form" name="13" class="form-horizontal" method="POST" action="{{ route('tenant.account.user.store', [$tenant->domain]) }}">
    {{ csrf_field() }}
    ...
    <div class="form-group bg-white clearfix p-15 m-r-0 m-l-0">
        <div class="pull-right" >
            ...
            <button class="btn btn-primary" type="submit"> <i class="fa fa-save"></i> Create User </button>
        </div>
    </div>     
            ...
            @foreach($roles->chunk(3) as $roleChunk)
                <div class="row">
                    @foreach($roleChunk as $role)
                        <div class="col-md-4">
                            <label class="checkbox" style="margin-left: 40px; margin-top: 5px; font-weight: 600; font-size: 15px;"> <input type="checkbox" name="roles[]" value="{{$role->id}}"> {{$role->label }}</label>
                        </div>
                    @endforeach
                </div>
            @endforeach

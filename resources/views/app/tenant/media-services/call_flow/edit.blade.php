...
@section('extra-script')
    ...
    <script type="text/javascript">
        ...
        let voicemails = <?= json_encode($voicemails); ?>;
        ...
        $("body").on('click', 'select.greeting-select', function () {
            ...
            switch($(this).find(":selected").attr('data-type')){
                case 'tts' :
                    $('.playback-message').html(`<p class="f-s-13" > ${$(this).find(":selected").attr('data-text')} </p> `);
                    $('.sound-path').val("");
        ...
        function voicemailOptions(){
            $('.destination').append(`<option value=""> &dash; &dash; &dash; Select Destination &dash; &dash; &dash; </option>`);
            voicemails.forEach(function(value, index){

                $('.destination').append(`<option value="${value.id}" data-type="voicemail" > ${value.title + " - " + value.user } </option>`); 

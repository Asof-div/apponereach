<form id="virtual_receptionist_edit_form" class="form-horizontal" name="13" action="" enctype='multipart/form-data' method='post'>

    {{csrf_field()}}
    <input type="hidden" name="receptionist_id" value="{{ $receptionist->id }}">
    <input type="hidden" name="destination" value="" id="destination">
    <input type="hidden" name="ivr" value="{{ $receptionist->menus->count() }}" id="ivr">
    <input type="hidden" name="ivr_sound_path" class="ivr-sound-path" value="{{ $receptionist->ivr_msg }}">
    ...
    <div class="col-md-7 col-sm-12 col-xs-12">
        ...
        <div class="form-group m-r-15 m-l-5 m-t-10">
            ...
            <input type="text" name="name" value="{{ $receptionist->name }}" placeholder="Name" class="form-control">
        </div>
    ...
    <div class="col-md-5 col-sm-12 col-xs-12 clearfix">
        ...
        <div class="form-group">
            ...
            <select class="form-control ivr-select-menu" name="ivr_menu">
                ...
                <optgroup label="Sound Files">
                    @foreach($sounds as $sound)
                        <option {{ $sound->id == $receptionist->play_media_id ? "selected" : "" }} value="{{ "s".$sound->id }}" data-type="sound" data-text="{{ asset('storage/'.$sound->path) }}">{{ $sound->title }}</option>
                    @endforeach
                </optgroup>
                <optgroup label="Text To Speech">
                    @foreach($txttosp->where('mime_type', 'greeting') as $tts)
                        <option {{ $tts->id == $receptionist->play_media_id ? "selected" : "" }} value="{{ "t".$tts->id }}" data-type="tts" data-text="{{ $tts->content }}">{{ $tts->title }}</option>
                    @endforeach
                </optgroup>
            </select>
            <p class="clearfix ivr-menu-tts hidden">
                ...
                <textarea name="ivr_tts" class="form-control hidden" rows="3">{{ $receptionist->ivr_msg }}</textarea>
            </p>

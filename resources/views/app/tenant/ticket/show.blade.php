...
@section('breadcrumb')
    ...
    <li class="active"> Details : {{ $ticket->ticket_no }}</li>
    ...
    <section class="">
        ...
        <div class="col-md-12 clearfix">
            <div class="m-b-20 pull-left text-left">
                <a class="btn btn-primary" href="{{ route('tenant.ticket.index', [$tenant->domain]) }}"><i class="fa fa-list"></i> List ticket </a>
                @if(Gate::check('tenant.ticket.create'))
                    <a class="btn btn-info" href="{{ route('tenant.ticket.create', [$tenant->domain]) }}"><i class="fa fa-plus"></i> New Ticket </a>
                @endif
            </div>
            ...
            <div class="m-b-20 pull-right text-right">
                ...
                @if ($ticket->is_default == false && Gate::check('tenant.ticket.update'))
                    ...
                    <span style="display: inline-block;">
                        <span class="dropdown">
                            ...
                            <ul class="dropdown-menu" role="menu">
                                ...
                                <li class="{{ strtolower($ticket->status) == 'closed' ? 'bg-success text-white' : '' }}"><a href="javascript:;" data-toggle="modal" data-target='#change_status_ticket_modal' data-status='closed'><i class="fa fa-check"></i> Close </a></li>
                                <li class="{{ strtolower($ticket->status) == 'resolved' ? 'bg-success text-white' : '' }}"><a href="javascript:;" data-toggle="modal" data-target='#change_status_ticket_modal' data-status='resolved'><i class="fa fa-check"></i> Resolved </a></li>
                            </ul>
                        </span>
                    </span>
                @endif

                @if ($ticket->is_default == false && Gate::check('tenant.ticket.delete'))
        ...
        <div class="row">
            @include('app.tenant.ticket.partials.details')     
            @include('app.tenant.ticket.partials.modal')     
        </div>
...
@section('extra-script')    
    ...
    <script>
        ...
        $('#change_status_ticket_modal').on('show.bs.modal', function(evt) {
            ...
            }else if('resolved' == button.data('status')){
                status = `<span class='label label-success'> Resolve </label> `;
            }

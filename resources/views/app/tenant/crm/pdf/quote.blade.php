...
<html>
	...
	<body>
		<header>
			...
			<h2 style="font-size: 13px;">Price Quotation - {{$quote->title}}</h2>
			
		</header>
		<div style="width: 100%; font-size: 11px;">
			<div style="width: 40%; float: left;">
				<div> <b>Quote Name:</b> {{$quote->title}} </div><br>
				<div> <b>Quote No #:</b> {{$quote->quote_no}} </div>
			</div>
			<div style="width: 25%; float: left;">
				<div> <b>Currency:</b>  {{ $quote->currency ? $quote->currency->name : '' }} </div> <br>
				<div> <b>Payment Terms:</b> {{ $payment }} </div>
			</div>
			<div style="width: 35%; float: left;">
				<div> <b>Created:</b> {{$quote->quote_date}} </div><br>
				<div> <b>Expiration Date:</b> {{ $quote->expiration_date }} </div><br>
		...
		<div style="width: 100%; font-size: 11px;">
			<div style="width: 30%; float: left; display: inline-block;">
				...
				<div> <b>{{$quote->account->name}}</b> </div>
				<div> {{ $quote->account->address }}, {{ $quote->account->city }} </div>
				<div> {{ $quote->account->state->name }}. </div>
				<div> {{ $quote->account->country->name }}. </div>
				<div>
				@if($quote->account->phone){{$quote->account->phone}} <br>@endif
				@if($quote->account->email){{$quote->account->email}} <br>@endif
				@if($quote->account->website){{$quote->account->website}} @endif
				</div>
			</div>
			<div style="width: 65%; float: right; display: inline-block;">
				<div style="line-height: 20px; "> <b>Contacts:</b> </div>
				<div style="">
				@foreach($quote->contacts as $contact)
					...
					@if($quote->items)
						...
						@foreach($quote->items as $index => $item)
							...
							<tr>
								...
								<td class="text-right"><span data-prefix></span><span class="currency-symbol">{{ $quote->currency ? $quote->currency->symbol : ''}}</span> <span>{{ number_format($item->uprice, 2) }}</span></td>
								<td class="text-right">@if($item->optional) Optional @else<span data-prefix></span><span class="currency-symbol">{{ $quote->currency ? $quote->currency->symbol : '' }} </span><span> {{ number_format($item->price, 2) }}</span>@endif</td>
			...
			<div class="right-4" style="clear: both; position: relative;">
				<table class="" style="clear: both; font-size: 12px;">
					<tr>
	                    ...
	                    <td class="text-right"> <span class="currency-symbol">{{ $quote->currency ? $quote->currency->symbol  : '' }}</span> <span class="grant-total">{{ number_format($quote->sub_total, 2) }}</span></td>
	                </tr>
					<tr>
	                    <th> <span contenteditable>Vat &nbsp; {{ $quote->vat_type == 'percentage' ? $quote->vat_rate.' %' : '' }} {{ $quote->vat_type == 'flat' ? ' Amount' : '' }} </span> </th>
	                    <td class="text-right"> <span class="currency-symbol">{{ $quote->currency ? $quote->currency->symbol  : '' }}</span> <span class="vat-total">{{ number_format($quote->vat, 2) }}</span></td>
	                </tr>
	                @if($quote->discount_rate > 0)
	                <tr>
	                    <th> <span contenteditable> Discount &nbsp; {{ $quote->discount_type == 'percentage' ? $quote->discount_rate.' %' : '' }} {{ $quote->discount_type == 'flat' ? ' Amount' : '' }} </span></th>
	                    <td class="text-right"> <span class="currency-symbol">{{ $quote->currency ? $quote->currency->symbol  : '' }}</span> <span class="discount-total">{{ $quote->discount }}</span></td>
	                </tr>
	                @endif
	                <tr>
	                    <th> <span contenteditable>Total </span></th>
	                    <td class="text-right"> <span class="currency-symbol">{{ $quote->currency ? $quote->currency->symbol  : '' }}</span> <span class="grant-total">{{ number_format($quote->grand_total, 2) }}</span></td>
	                </tr>
				</table>
			</div>
		</article>

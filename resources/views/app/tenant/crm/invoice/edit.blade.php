    ...
    <div class="col-md-12 col-sm-12 col-xs-12 p-0">
        <div class="panel panel-default">
            ...
            <div class="panel-body" style="min-height: 400px;">
                ...
                @include('app.tenant.crm.invoice.partials.edit_form')
...
@section('extra-script')
    ...
    <script>
        ...
        $('body').on('submit', '#edit_invoice_form',  function(event){
            event.preventDefault();
            let form = document.getElementById('edit_invoice_form');
            let formData = new FormData(form);

            url = "{{ route('tenant.crm.invoice.store', [$tenant->domain]) }}"; 

            let success = function(data){
                printFlashMsg(data.success);
                setTimeout(function(){ 
                    window.location = "{{ route('tenant.crm.invoice.edit', [$tenant->domain, $invoice->id]) }}";
                }, 5000);  
        ...
        $('body').on('click', '.add-item', function(e){
            ...
            $('.invoice-currency').text($("select[name='currency'] option:selected").attr('data-symbol') );
            ...
            if( (desc.length > 5) & optional & ($.isNumeric(uprice) & parseInt(uprice) >= 0 ) & ($.isNumeric(quantity) & parseInt(quantity) >= 1 ) ){
                ...
                $('.table .invoice-items').append(`<tr> 
                    ...
                    <td> <span class='invoice-currency'></span> ${ (parseFloat(uprice)).toLocaleString()} </td>
                    <td> Optional </td>
                ...
                cal($('.table .invoice-items').find('tr'));
                ...
                $('.table .invoice-items').append(`<tr> 
                    ...
                    <td> ${ (parseInt(quantity)).toLocaleString() } </td>
                    <td> <span class='invoice-currency'></span> ${ (parseFloat(uprice)).toLocaleString()} </td>
                    <td> <span class='invoice-currency'></span> ${ (parseInt(quantity) * uprice).toLocaleString() } </td>
            ...
            if(currency){
                $('.invoice-currency').html(currency.symbol);
        ...
        $('body').on('change', ".invoice-summary select[name='vat_type']", function(e){

            calVat();

        });

        $('body').on('change', ".invoice-summary input[name='vat_rate']", function(e){
        ...
        $('body').on('change', ".invoice-summary select[name='discount_type']", function(e){

            calDiscount();

        });

        $('body').on('change', ".invoice-summary input[name='discount_rate']", function(e){
        ...
        function cal(element){
            ...
            $('.invoice-summary .sub-total').text(amount.toLocaleString());
            let discount = $('.invoice-summary .discount-total').text().replace(/,/g,'');
            let gtotal = parseFloat(amount - discount).toFixed(2);
            gtotal = parseFloat(gtotal);
            $('.invoice-summary .grand-total').text(gtotal.toLocaleString());
            calDiscount();
            ...
            if(type == 'percentage'){
                ...
                if( $.isNumeric( num ) && num <= total && num >= 0){
                    ...
                    if(!$.isNumeric(num)){
                        ...
                        fail.push('Vat Value must not be greater than Sub Total '+currency+''+$('.invoice-summary .sub-total').text() );
                    }
                    printErrorMsg(fail, 8000);
                }
            }   
        ...
        function calDiscount(){
            ...
            let type = $('.invoice-summary .discount-type').val();
            let currency = $("select[name='currency'] option:selected").attr('data-symbol');
            if(type == 'percentage'){
                let num = parseFloat($('.invoice-summary .discount-val').val());
                if( $.isNumeric( num ) & parseInt(num) >= 0 & parseInt(num) <= 100 ){
                    let total = $('.invoice-summary .sub-total').text().replace(/,/g,'');
                    
                    let discount = parseFloat( total * parseFloat(num / 100).toFixed(2));
                    let sdiscount = total - discount;
                    $('.invoice-summary .discount-total').text(discount.toLocaleString());
                ...
                if( !isNaN( num ) && num <= total && num >= 0 ){
                    ...
                    $('.invoice-summary .discount-total').text(num.toLocaleString());

                    let gtotal = parseFloat(sdiscount).toFixed(2);
                    gtotal = parseFloat(gtotal);
                    $('.invoice-total .grant-total').text(gtotal.toLocaleString());
                    calVat();
                    ...
                    if(!$.isNumeric(num)){
                        ...
                        fail.push('Discount Value must not be greater than Sub Total '+currency+''+$('.invoice-summary .sub-total').text());
                    }
                    printErrorMsg(fail, 8000);
                }
            }   
        ...
        function calBalance(gtotal){
            ...
            if( $.isNumeric(deposit) & ( parseInt(deposit) <= gtotal) ){
                ...
                $('.invoice-summary .balance-due').text(balance.toLocaleString());
                $('.invoice-summary .deposit-total').text(deposit.toLocaleString());
                ...
                $('.invoice-summary .deposit-total').text('0.00');
                let fail = new Array();

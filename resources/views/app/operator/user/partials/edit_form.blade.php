...
<form id="user_form" name="13" class="form-horizontal" method="POST" action="{{ route('operator.user.update') }}">
    {{ csrf_field() }}
    ...
    <div class="p-20 col-md-12 bg-white m-b-15">
        ...
        <div class="col-md-6 col-sm-6">
            ...
            <div class="form-group">
                ...
                <div class="col-md-8">
                    <input type="text" name="job_title" class="form-control" required="required" value="{{ $user->job_title }}">
                </div>
            </div>
            ...
            @foreach($roles->chunk(3) as $roleChunk)
                <div class="row">
                    @foreach($roleChunk as $role)
                        <div class="col-md-4">
                            <label data-toggle="tooltip" data-placement="top" title="{{$role->description}} " class="checkbox" style="margin-left: 40px; margin-top: 5px; font-weight: 600; font-size: 15px;"> <input type="checkbox" {{ $user->hasRole($role->name) ? 'checked' : '' }} name="roles[]" value="{{$role->id}}"> {{$role->label }}</label>
                        </div>
                    @endforeach
                </div>
            @endforeach
    ...
    <div class="col-md-12 bg-white clearfix p-15 m-t-20 m-r-0 m-l-0">
        <div class="pull-right" >
            ...
            <button class="btn btn-success" type="submit"> <i class="fa fa-save"></i> Update User </button>
        </div>
    </div>
    @else
